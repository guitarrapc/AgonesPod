...
using System.Text;
using AgonesPod.Internal.Utf8Json.Internal;

#if NETSTANDARD
using System.Runtime.CompilerServices;
#endif

namespace AgonesPod.Internal.Utf8Json
{
    ...
    internal struct JsonReader
    {
        ...
        public float ReadSingle()
        {
            SkipWhiteSpace();
            int readCount;
            var v = AgonesPod.Internal.Utf8Json.Internal.DoubleConversion.StringToDoubleConverter.ToSingle(bytes, offset, out readCount);
            if (readCount == 0)
            {
                throw CreateParsingException("Number Token");
            }
        ...
        public double ReadDouble()
        {
            SkipWhiteSpace();
            int readCount;
            var v = AgonesPod.Internal.Utf8Json.Internal.DoubleConversion.StringToDoubleConverter.ToDouble(bytes, offset, out readCount);
            if (readCount == 0)
            {
                throw CreateParsingException("Number Token");
            }
